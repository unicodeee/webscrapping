...
def scrap_this_page(container, path, file_name):
    ...
    for group in questionGroups:
        ...
        if correct_answers and correct_answers[0].find("div", class_="answer_text"):
            temp_question.correct_answer = correct_answers[0].find("div", class_="answer_text").text.strip()
        else:
            temp_question.correct_answer = "UNKNOWN"
            print(f"[WARN] Missing correct answer in file '{file_name}' for question: {temp_question.question[:80]}...")
